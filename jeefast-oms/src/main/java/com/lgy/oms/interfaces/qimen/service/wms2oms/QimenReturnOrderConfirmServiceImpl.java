...
public class QimenReturnOrderConfirmServiceImpl implements QimenService {
    ...
    public QimenResponse requestExec(QimenParam qimenParam) {
        ...
        ReturnOrderConfirmRequest request;

        /** 1. XML转换 检查参数 */
        try {
            if (QimenConstants.XML.equalsIgnoreCase(qimenParam.getFormat())) {
                request = JaxbUtil.converyToJavaBean(qimenParam.getData(), ReturnOrderConfirmRequest.class);
            } else {
                request = JSON.parseObject(qimenParam.getData(), ReturnOrderConfirmRequest.class);
            }
        } catch (Exception e) {
