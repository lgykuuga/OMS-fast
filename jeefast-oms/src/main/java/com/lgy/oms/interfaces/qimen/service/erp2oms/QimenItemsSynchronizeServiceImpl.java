...
public class QimenItemsSynchronizeServiceImpl implements QimenService {
    ...
    public QimenResponse requestExec(QimenParam qimenParam) {
        ...
        ItemsSynchronizeRequest request;

        /** 1. XML转换 检查参数 */
        try {
            if (QimenConstants.XML.equalsIgnoreCase(qimenParam.getFormat())) {
                request = JaxbUtil.converyToJavaBean(qimenParam.getData(), ItemsSynchronizeRequest.class);
            } else {
                request = JSON.parseObject(qimenParam.getData(), ItemsSynchronizeRequest.class);
            }
        } catch (Exception e) {
